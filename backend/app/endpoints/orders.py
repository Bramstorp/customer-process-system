...
@order_routes.post(
    "/create-orders", tags=["orders"], status_code=201, description="Create new orders"
)
def create_order(order: OrderCreate):
    ...
    if not customer:
        return JSONResponse(
            status_code=HTTP_404_NOT_FOUND, content="Customer not found"
        )
...
@order_routes.get(
    "/order/{order_id}",
    response_model=OrderReadWithCustomer,
    tags=["orders"],
    status_code=201,
...
@order_routes.get(
    "/orders", tags=["orders"], status_code=201, description="Get all orders"
)
def get_orders():
